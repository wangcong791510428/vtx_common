...
encrypt(Data, Key) when is_binary(Data) and is_binary(Key) ->
    case byte_size(Key)< 16 of
        true ->
            {error, <<"key too short">>};
        false ->
            Key16 = binary:part(Key, 0, 16),
            int_list_to_binary( encrypt_int_list(
                binary_to_int_list(Data, false), binary_to_int_list(Key16, false)), false)
    end;

encrypt(Data, Key) when is_list(Data) and is_list(Key) ->
    case length(Key)< 16 of
        true ->
            {error, "key too short"};
        false ->
            Key16 = string:substr(Key, 1, 16),
            int_list_to_binary( encrypt_int_list(
                binary_to_int_list(Data, false), binary_to_int_list(Key16, false)), false)
    end.
...
decrypt(Data, Key) when is_binary(Data) ->
    case length(Key) < 16 of
        ...
        false ->
            Key16 = string:substr(Key, 1, 16),
            Result = int_list_to_binary(decrypt_int_list(
                    binary_to_int_list(Data, false), binary_to_int_list(Key16, false)), false),
            {ok, Result}
    end.
